...
export async function verifyOTP(formData: FormData) {
	...
	if (!userData.user) {
		...
		return {
			error: "User data not found after confirmation. Please try logging in.",
		};
	}
	...
	if (dbError) {
		...
		console.warn(
			"Failed to update custom users table, but auth update was successful"
		);
	}
	...
	if (signInError) {
		...
		return {
			error:
				"Account confirmed, but failed to sign in. Please try logging in manually.",
		};
	}
